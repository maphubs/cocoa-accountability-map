  ...
  const onSearch = (val) => {
    if (!val) {
      ...
      if (coopBBox && primaryMapState?.state?.map?.map) {
        primaryMapState.state.map.map.fitBounds(coopBBox, {
          padding: 25,
          curve: 3,
          speed: 0.6,
          maxZoom: 10,
      ...
      <Row style={{ padding: '10px', height: '100%', width: '100%' }}>
        ...
        <div style={{ height: '100%', width: '100%', marginTop: '5px' }}>
          {searchResults.length > 0 && (
            <>
              <span>
                {searchResults.length} {t('Result(s) Found')}
              </span>
                  ...
                  renderItem={(coop) => (
                    <List.Item>
                      <Row
                        ...
                        onClick={() => {
                          ...
                          if (bbox && primaryMapState?.state?.map?.map) {
                            primaryMapState.state.map.map.fitBounds(bbox, {
                              padding: 25,
                              curve: 3,
                              speed: 0.6,
                              maxZoom: 15,
                      ...
                      >
                        ...
                        >
                          {coop.properties['Coop Acronym']}
                        </div>
                        <div
                          style={{ bottom: '2px', left: 0, fontSize: '9px' }}
                        >
                          {coop.properties['Name of Cooperative']}
                        </div>
                      </Row>
                    </List.Item>
                  )}
          ...
          {searchResults.length === 0 && (
            <Empty
              ...
              description={t('Search Cooperatives')}
            />
          )}
        </div>
      </Row>
